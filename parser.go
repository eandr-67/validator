...
func Parse(reader io.Reader, validator Validator) (result map[string]any, err *errs.Errors) {
	var raw any
	if json.NewDecoder(reader).Decode(&raw) != nil {
		err = &errs.Errors{}
		err.Add("", ErrMsg[CodeFormatIncorrect])
		return nil, err
	}
...
// Возвращает результат обработки и список ошибок.
// Синтаксический сахар для того, чтобы не загромождать код API.
func ParseString(str string, validator Validator) (result any, err *errs.Errors) {
	var data any
...
func anyToMap(data any, err *errs.Errors) (map[string]any, *errs.Errors) {
	...
	err.Add("", ErrMsg[CodeTypeIncorrect])
	return nil, err
}
