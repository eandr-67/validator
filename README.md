...
```go
map[string][]sting{".paginator.size": ["value"], ".key[2]": ["is_null"]}
```
...
1. Выполнение начального (before) набора действий.
2. Выполнение обработчика (handler), применяющего валидаторы к составным частям значения.
3. Выполнение конечного (after) набора действий.

Соответственно, вместо метода `Append` построителей базового пакета построители валидаторов структурных данных имеют
два метода: `Before` и `After`, добавляющие 